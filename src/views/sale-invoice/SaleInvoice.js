...
const SaleInvoices = () => {
        ...
        // Function to export stock requests to Excel
        const handleExportStockRequests = async() => {
            try {
                ...
                // Add reseller filter if present in active search
                ...
                let fileName = 'stock_requests_report';
                ...
                // Add reseller name to filename if filter is applied
                if (activeSearch.reseller) {
                ...
                console.error('Error exporting stock requests:', error);

                // Handle different error types
                if (error.response) {
                    // The request was made and the server responded with a status code
                    if (error.response.status === 404) {
                        showAlert('Export endpoint not found. Please check API configuration.', 'danger');
                    } else if (error.response.status === 500) {
                        showAlert('Server error while generating export. Please try again later.', 'danger');
                    } else if (error.response.data && error.response.data.message) {
                        showAlert(error.response.data.message, 'danger');
                    } else {
                        showAlert('Error generating export file. Please try again.', 'danger');
                    }
                } else if (error.request) {
                    // The request was made but no response was received
                    showAlert('No response from server. Please check your network connection.', 'danger');
                } else {
        ...
        const openExportModal = () => {
            // Pre-fill with current search dates if any
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    invoices.forEach(invoice => {
                        ...
                        // Get challan numbers from stockRequestIds
                        const challanNumbers = invoice.stockRequestIds?.map(sr => sr.challanNo).filter(Boolean).join(', ') || '';
        ...
        <table class="main-border" style="margin-top:10px;">
          ...
          <tbody>
            ${hsnSummary.map(hsn => `
             ...
             <tr>
           <td colspan="7"><b>Taxable Amount (in words):</b> <i>${numToWords(parseFloat((cgst + sgst).toFixed(2)))}</i></td>
            </tr>
          </tbody> 
        </table>
          <button class="print-button no-print" onclick="window.print()">🖨️ Print</button>
  ...
  return (
    <div>
      ...
      {/* Export Modal for Stock Requests */}
      <CModal visible={exportModalVisible} onClose={() => setExportModalVisible(false)} size="md">
        <CModalHeader>
          <CModalTitle>Export Stock Requests Report</CModalTitle>
        </CModalHeader>
        
        <CModalBody>
          <div className="form-group mb-3">
            <CFormLabelModal htmlFor="exportStartDate">Start Date (Optional)</CFormLabelModal>
            <CFormInput
              type="date"
              id="exportStartDate"
          ...
          <div className="form-group mb-3">
            <CFormLabelModal htmlFor="exportEndDate">End Date (Optional)</CFormLabelModal>
            <CFormInput
              type="date"
              id="exportEndDate"
              value={exportEndDate}
          ...
          {(activeSearch.reseller || activeSearch.center) && (
        ...
        <CModalFooter>
          ...
          <CButton color="primary" onClick={handleExportStockRequests} disabled={exportStockLoading}>
            {exportStockLoading ? (
              <>
            ...
            <CTabPane visible={activeTab === 'stockRequests'}>
              <div className="d-flex justify-content-between mb-3">
                <div>
                  ...
                  {/* New Export Button for Stock Requests */}
                  <CButton 
                    size="sm" 
                    className="action-btn me-1"
                    onClick={openExportModal}
                    disabled={exportStockLoading}
                  >
                    <i className="fa fa-fw fa-file-excel me-1"></i>
                    {exportStockLoading ? ' Exporting...' : ' Export Stock Report'}
                  </CButton>
                  ...
                  {(isSearchActive() && selectedChallans.length > 0) && (
                    <CButton 
                      ...
                      className="action-btn me-2"
                      onClick={() => {
                        // This will open the meta data modal for invoice generation
                        // You need to implement the meta data modal or directly call handleGenerateInvoice
            ...
            <CTabPane visible={activeTab === 'invoices'}>
              <div className="d-flex justify-content-between mb-3">
                <div>
                  ...
                  >
                    <i className="fa fa-fw fa-file-excel"></i>
                    {invoiceLoading ? ' Exporting...' : ' Export Invoices'}
                  </CButton>
                </div>
