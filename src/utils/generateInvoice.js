...
export const generateInvoice = (reseller, centers, challans) => {
  ...
  // FIXED: Using salePrice instead of rate
  const allProducts = challans?.length
    ? challans.flatMap(c =>
        c.products.map(p => ({
          name: p.product?.productTitle || 'Item Name',
          qty: p.approvedQuantity || 0,
          rate: p.product?.salePrice || 0,  // 👈 CHANGED: rate → salePrice
          hsn: p.product?.hsnCode || p.product?.productCode || '00000000', //  FIXED: Use hsnCode
          amount: (p.approvedQuantity || 0) * (p.product?.salePrice || 0), // 👈 CHANGED: Use salePrice
        })),
      )
    : [
        { name: 'Fiber Cable 12F', qty: 10, rate: 784.3, hsn: '90011000', amount: 7843 },
  ...
  // Border layout for main header block (same as before)
  const headerBlock = {
    table: {
      widths: ['40%', '30%', '30%'],
      body: [
    ...
    table: {
      ...
      body: [
        [
          ...
          { text: 'HSN/SAC', style: 'tableHeader' },
          { text: 'Quantity', style: 'tableHeader' },
          { text: 'Rate (Sale Price)', style: 'tableHeader' }, // 👈 Updated label
          { text: 'Amount', style: 'tableHeader' },
        ],
        ...allProducts.map((p, i) => [
